    ...
    public function generateActiveField($attribute, $options)
    {
        ...
        if ($tableSchema === false || !isset($tableSchema->columns[$attribute])) {
            if (preg_match('/^(password|pass|passwd|passcode)$/i', $attribute)) {
                return "\$form->field(\$model, '$attribute', $options)->passwordInput()";
            } else {
        ...
        if ($column->phpType === 'boolean') {
            return "\$form->field(\$model, '$attribute', $options)->checkbox()";
        } elseif ($column->type === 'text') {
            return "\$form->field(\$model, '$attribute', $options)->textarea(['rows' => 6])";
        } else {
            ...
            if (is_array($column->enumValues) && count($column->enumValues) > 0) {
                ...
                return "\$form->field(\$model, '$attribute' , $options)->dropDownList("
                    . preg_replace("/\n\s*/", ' ', VarDumper::export($dropDownOptions)).", ['prompt' => ''])";
            } elseif ($column->phpType !== 'string' || $column->size === null) {
                return "\$form->field(\$model, '$attribute', $options)->$input()";
            } else {
                return "\$form->field(\$model, '$attribute', $options)->$input(['maxlength' => true])";
            }
        }
    }
